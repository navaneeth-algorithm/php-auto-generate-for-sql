...
class GeneratePHPMySQLQuery:
    def __init__(self,attributeObj):
        ...
        self.__ajaxTemplate=""
        self .__insertTemplate = "$query='INSERT INTO `"+self.__tableName+"`("
        self.__executeTemplate = "$stmt=$conn->prepare($query);\n$stmt->execute(["
...
attributes = AttributeExtraction(tableExtraction,"Profile")
